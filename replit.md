...
- Required Replit Secrets: `MONGODB_URI` (MongoDB Atlas connection string), `SESSION_SECRET` (signs the login session cookie)
- Pipeline API keys (OpenAI, Apify, Apollo, PageSpeed) are entered via the dashboard **Settings** page and saved to `pipeline_settings.json` — they do not need to be Replit Secrets
- The API server starts in a "not_configured" mode when `MONGODB_URI` is absent — the dashboard shows a setup message and becomes fully functional once it is provided
